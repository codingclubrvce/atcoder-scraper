...
def parse_submission(submit_link):
    page = requests.get(submit_link)
    pageContent = bs4.BeautifulSoup(page.content, "html.parser")
    table = pageContent.find("div",class_="panel")
    elements = table.find_all("tr")
    statusRow = elements[6]
    timeRow = elements[7]
    memoryRow = elements[8]

    return {
    'status': statusRow.find("td").text,
    'time': timeRow.find("td").text,
    'memory': memoryRow.find("td").text
    }
